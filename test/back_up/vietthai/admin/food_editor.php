...
require_once("includes/application_top.php");
	...
	function load_products($MENU)
	{
		...
		if($food != null)
		{
			while($row = tep_db_fetch_array($food))
			{
				...
				echo '<td id="'.$row['ID'].'"><a href="javascript:void(0)" onclick="delete_food(' . $row['ID'] . ')">Delete</a> &nbsp;&nbsp;&nbsp;<a href="javascript:void(0)" onclick="update_food('.$row['ID'].','."'".$row['NAME']."'".','."'".$row['PRICE']."'".','."'".$row['DESCRIPTION']."'".')">Update</a></td>';
				echo '</tr>';
			}
		}
		else
	...
	if($_POST['action'] == 'add_food')
	{
		...
		$query = "insert into foods(NAME,DESCRIPTION,MENU,PRICE) values('$NAME', '$DESCRIPTION', '$MENU', '$PRICE');";
	...
	if($_POST['action'] == 'update_food')
	{
		...
		$PRICE = $_POST['food_price'];
		tep_db_query("update foods set foods.NAME ='" .$NAME. "', foods.DESCRIPTION = '" . $DESCRIPTION . "', foods.PRICE ='".$PRICE."', foods.MENU = '".$MENU_ID."' where foods.ID = '" . $ID . "' ;");
